...
def randomize_env(env):
    env.masspole = np.random.choice(np.arange(0.05, 0.151, 0.025))
    env.length = np.random.choice(np.arange(0.3, 0.71, 0.1))

def get_domains(env):
    return [env.masspole, env.length]
...
def train(env_class, agent, flags, logger):
    ...
    buffer = RolloutBuffer(flags.buffer_len, ['states', 'actions', 'domains'], obs_preprocessing)
    ...
    while timestep < flags.timesteps:
        ...
        for i, (ns, r, done, info) in enumerate(steps):
            envs_history[i].append(envs_states[i].copy(), actions[i])
            if len(envs_history[i]) > flags.len_history: ############
                buffer.append({k:v for k,v in zip(['states', 'actions', 'domains'], 
                                                [envs_history[i]['state'].copy(), envs_history[i]['action'], get_domains(envs[i])])}, device=flags.device)
            envs_states[i] = ns
            envs_reward[i] += r
            envs_timesteps[i] += 1
            if envs_timesteps[i] > flags.horizon:
...
if __name__ == "__main__":
    ...
    dummy_env = env_class()
    agent = UP_OSI(dummy_env.observation_space, dummy_env.action_space, flags, logger=logger)
